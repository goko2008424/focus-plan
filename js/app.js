...
(function () {
  ...
  function aboutModal() {
    const bodyHTML = '' +
      ...
      '<li><b>🎯 小时计划</b>：今天页顶部的小时计划卡片——每段先<b>提前定</b>：几点开始、多长、必须/理想/拓展各做多久、以及<b>达标给多少奖励积分</b>。<b>计时权威</b>：到点自动结算、只把 [开始→到点] 窗口内的实际学习计入、超时补做不算、未达标＝0积分。<b>中途消耗自查</b>：达标先自查有没有看手机/刷屏，有则按设置%扣（默认100%＝扣光），休息结束同样自查。<b>休息系统</b>：段结束强制选「继续做 / 去休息」；休息可定 类型（纯休息/吃饭/睡觉）+时长+提前填「好好休息」积分，可提前结束、到点自动结束，结束强制开下一段。<b>秒级计时 + 复盘</b>：小任务记到秒可累加；开始段可关联任务，历史页每条可写感想；可「📤 导出复盘给AI」提取整天/整年；可「📌 预定明天的段」第二天一键开跑。<b>☕ 小休×小时代</b>：学习中途点「☕ 小休」，这一段的倒计时同步暂停，休多久这段就顺延多久（休息不吃窗口）。<b>到点自动结算不怕忘</b>：到点自动结算时，还在做、还没点结算的任务时间也会一并算进这一段，学够了就照常加分。<b>🪟 计时悬浮窗能拖出浏览器</b>：点悬浮窗右上「⇱ 拖出」，它变成一个独立小窗口——可拖到屏幕任意位置、永远置顶，切到别的软件甚至最小化浏览器，计时照常走字。小窗下方叠着一个可收起的抽屉：显示本组进度、剩几题，还能「✅ 做完这题」→「▶ 下一题」一键连做同一个任务组，不用回网页来回切；整组做完会弹「☕ 去休息 / ▶ 再安排点」。小时代和休息到点会发系统通知（网页挂后台也能提醒你回来安排）。手机浏览器不允许网页开小窗，会自动用页内悬浮窗并开启防息屏。</b><b>🎓 听课三步（长在任务里）</b>：<b>没有单独的听课页了</b>——每条任务右边都有 🎧（今天、明天都有），点一下三步面板就直接展开在这条任务下面，不用跳页。面板上随手能改预算（预习/听课/整理/大奖分，会记住你上次用的）。流程：预习封顶只读目标与总结 → 课前准备清单 → 听课空白纸记重点 → 课后用自己的逻辑重构并写核心逻辑链，三步齐了才发大奖——把"被动复制"变成"主动筛选"（方法来源：<a href="https://www.bilibili.com/video/BV1DA411N78G" target="_blank" style="color:var(--primary)">📺 B站【让我效率翻倍的听网课方法！】</a>，高度个性化，建议按自己节奏改预算）。<b>📅 日历 + 间隔重做</b>：完成任务时可选「过几天再做一次」并写下重做标准（如全对/限时解出）；日历页能往前翻历史任务、往后规划任意一天，忘了安排、手误安排都能 🔁 改期救回；附一年份学习热力图，学得越久颜色越深。熬夜跨天：结束今天会自动改结算昨天，没做完的任务可当场补勾划掉；🌱 长期拓展严格模式下没做完先挂账宽限一次，到期没补完才扣分。<b>🎙 笔记工坊</b>：用桌面「开始录课.bat」+「笔记网站.bat」把网课录音变成 AI 课程笔记，在「🎙 笔记」页直接看。专治「没指标就休息太久 / 中途跑去做别的」。</li>' +
      ...
      '<li><b>📅 日历 + 🔁 间隔重做</b>：整月日历，过去显示学了多久、未来显示已排任务；点任意一天像「明天的任务」一样编辑。完成任务时可安排「过几天再做一次」并写下<b>重做标准</b>（如全对/限时解出）；忘了安排、手误安排都能 🔁 改期救回；附一年学习热力图、近30天柱状图和当天时间构成饼图。</li>' +
      '<li><b>🌱 长期拓展严格模式（设置可关）</b>：拓展任务不顺延，没做完先挂账<b>宽限一次</b>——下次结算前补勾划掉就不扣，到期没补完才按分值扣；熬夜跨天会自动结算昨天、支持当场补勾。</li>' +
      ...
      '<li><b>🏃 运动强化</b>：新增「运动」页——运动安排<b>任选日期</b>（今天 / 明天 / 自选日期）都能写，两步操作：➕ 新建一组运动 → ➕ 这组加一项（每项可设 做完+分 / 没做−分，组还能设整组奖励）。今天临时想起来的，今日运动页「➕ 临时加一项」直接加；做完一项点它得积分，没做到的结算统一扣分（扣多少由你定），编辑今天的安排不会弄丢已完成/已扣的记录。</li>' +
      ...
      '<li><b>结束今天</b>：必须/理想的未完成任务勾选顺延到明天（无惩罚）；实际做完但没来得及勾的当场补勾划掉；拓展按严格模式挂账宽限。结算完去「明天」页看看明天的任务。</li>' +
      '<li><b>常看历史</b>：看周图、月线和流水账——「看得见成果」是这个方法能否长期生效的关键。</li>' +
      '</ol>' +

      '<h4 style="margin:14px 0 6px;color:#2d3a4a">📋 默认规则（全部可在设置里改）</h4>' +
      '<p style="font-size:13.5px">理想每条 +10 分 · 拓展每条 +5 分 · 小题默认 +10 分 · 保底奖励 积分+10 · 完美奖励 积分+20 · 未完成顺延无惩罚 · 拓展可追加 · 长期拓展严格模式 默认开（挂账宽限+补勾，可关） · 六套主题随便换</p>' +
