...
(function () {
  ...
  function aboutModal() {
    const bodyHTML = '' +
      ...
      '<li><b>⏰ 到点自动结算</b>：设置里定一个时刻（默认 23:59），到点自动做和「🏁 结束今天」一样的事：把没做完的长期拓展复制到第二天、该扣的扣掉、其余未完成任务按「顺延」开关搬走，然后那天的账就封存了；<b>结算点设在凌晨（如 01:30）也行</b>——它算作「前一天的收工点」，适合熬夜到半夜；到点时页面没开着也没关系，下次打开会立刻补结算一次。<b>还有任务在计时时不结算</b>（不打断你），停下来了才自动结。</li>' +
