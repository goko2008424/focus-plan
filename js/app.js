...
(function () {
  ...
  function aboutModal() {
    const bodyHTML = '' +
      ...
      '<li><b>🔥 连续学习 & 小休</b>：任务页顶部实时显示「已连续学习」；学习时可点「☕ 小休」（休息单独算、不占学习，默认5分钟），休息结束问「休息中有没有消耗性的事」，有则记一次、结束今天统一扣分；休息后连续学习归零。</li>' +
      '<li><b>🏃 运动强化</b>：新增「运动」页——前一天写好明天的运动项目（分组+每项设积分），今日做完一项点它得积分；没做到的结算统一扣分（扣多少由你定）。</li>' +
