...
(function () {
  ...
  function aboutModal() {
    const bodyHTML = '' +
      ...
      '<li><b>📅 日历 + 🔁 间隔重做</b>：整月日历，过去显示学了多久、未来显示已排任务；点任意一天像「明天的任务」一样编辑。<b>每道题（含任务组里的题）后面都有 🔁</b>，单独安排到另一天并带着组归属；整条任务也能安排，可选用不用连小任务/任务组一起带。「明天」页还能 📋 从其他天转移任务过来：<b>每条能展开看清楚昨天哪道题做完了、哪道没做</b>，<b>默认只带没做完的题</b>（做过的不用再做一遍），也能一键「全部都带」；每道题可单独勾选，每条任务还能改<b>目标分类栏</b>——<b>拓展没做完的直接搬成第二天的「必须」</b>。搬过来的一律是初始状态。忘了安排、手误安排都能 🔁 改期救回；附一年学习热力图、近30天柱状图和当天时间构成饼图。</li>' +
      '<li><b>🌱 长期拓展严格模式（设置可关、扣分倍数可调）</b>：拓展任务不顺延，没做完先「挂账」，<b>一分不扣</b>；下次点「🏁 结束今天」时会弹窗给你<b>最后一次补勾机会</b>——确实补做完了就勾上，划掉并且<b>积分照发</b>；到那时还没补完才真扣，扣多少 = 任务分值 × 你在「设置 → 🌱 长期拓展任务」里定的倍数（1 倍 = 5 分的任务扣 5 分，填 0 = 只记账不扣分）。结算弹窗里的「☑ 补记」也一样：补记完成照发积分，不会又被挂账。熬夜跨天会自动结算昨天、支持当场补勾。</li>' +
